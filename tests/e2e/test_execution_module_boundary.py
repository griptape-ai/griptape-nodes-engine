...
class TestAllThreeMechanismsTogether:
    ...
    @pytest.mark.asyncio
    async def test_the_device_answer_reaches_the_execution_module(self, tmp_path: Path) -> None:
        """The node hands the engine's device answer to the execution module, which uses it.

        The point of the arrangement: the node never imported a framework to find an accelerator,
        and never imported the execution module to run against one.
        """
        current_engine().library_manager._is_worker = True
        _register(tmp_path)

        node = LibraryRegistry.create_node(node_type="BoundaryNode", name="both", specific_library_name=LIBRARY)
        current_engine().object_manager.add_object_by_name("both", node)
        result = await current_engine().ahandle_request(
            ExecuteNodeRequest(
                node_name="both",
                parameter_values={},
