...
class TestLibrariesWithoutExecutionDependencies:
    ...
    def test_dropping_the_field_retires_an_existing_execution_environment(self, tmp_path: Path) -> None:
        """The layout follows the manifest, not the install history.

        The execution environment reaches a worker by existing on disk, so one left behind by an
        earlier manifest would still be imported from, carrying pins nothing declares any more.
        """
        wheel_dir = tmp_path / "wheels"
        _build_both_wheels(wheel_dir)
        library_dir = tmp_path / "library"
        library_json = _materialize_library(
            library_dir, wheel_dir=wheel_dir, name="Exec Dep Retired", exec_dependencies=[]
        )
        # Stands in for an environment an earlier version of this manifest built.
        stale_venv = library_dir / ".venv-exec"
        stale_venv.mkdir(parents=True)
        (stale_venv / "pyvenv.cfg").write_text("home = /nowhere\n")
