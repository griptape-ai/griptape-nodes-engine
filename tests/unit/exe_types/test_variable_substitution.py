...
def _mock_gn(variables: dict, *, connected_params: set[str] | None = None) -> AbstractContextManager:
...
class TestVariableSubstitutionFallbacks:
    ...
    def test_flow_manager_exception_treats_param_as_unconnected(self) -> None:
        """If get_connections() raises, assume no connection and proceed with substitution."""
        node = MockNode(name="mock_node")
        node.add_parameter(_make_str_param("text", "{SHOT}"))
        node.parameter_values["text"] = "{SHOT}"
        ...
        mock_gn = MagicMock()
        mock_gn.NodeManager.return_value.get_node_parent_flow_by_name.return_value = "test_flow"
        mock_gn.VariablesManager.return_value.get_variables_for_macro_resolution.return_value = {"SHOT": "sc001"}
        mock_gn.SecretsManager.return_value = MagicMock()
        mock_gn.FlowManager.return_value.get_connections.side_effect = RuntimeError("no flow")
