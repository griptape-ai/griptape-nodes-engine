...
class TestGetDisplayableImageBytesDiskCache:
    ...
    async def test_only_if_stale_regenerates_when_source_modified(
        ...
        # Rewriting in-test lands inside MTIME_MATCH_TOLERANCE_SECONDS, and the re-encoded
        # JPEG is the same size, so staleness needs an mtime beyond the tolerance window.
        future_time = time.time() + 100
        os.utime(test_image_path, (future_time, future_time))
