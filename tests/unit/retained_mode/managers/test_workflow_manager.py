...
class TestWorkflowManager:
    ...
    def test_workflow_metadata_is_private_round_trip(self) -> None:
        """is_private defaults to False, parses from headers, and survives model_dump()."""
        base_kwargs = {
            "name": "wf",
            "schema_version": WorkflowMetadata.LATEST_SCHEMA_VERSION,
            "engine_version_created_with": "",
        ...
        assert WorkflowMetadata(**base_kwargs).is_private is False

        # Parsed from a header table (mirrors the TOML [tool.griptape-nodes] path).
        parsed = WorkflowMetadata.model_validate({**base_kwargs, "is_private": True})
        assert parsed.is_private is True

        # Survives model_dump() so it reaches list_workflows() -> the GUI.
        assert parsed.model_dump()["is_private"] is True
