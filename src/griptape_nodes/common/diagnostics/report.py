...
class ConfigFileDiagnostics(BaseModel):
    ...
    Attributes:
        ...
        contributes: Whether this layer feeds the merge. Lower than ``exists`` when the
            workspace directory *is* the project directory: both layers name the same
            file, and it is read once, as ``project``.
        parse_error: Why the file failed to parse, when it exists but is not valid JSON.
            None for a missing file, which tells "no such file" apart from "file is
            broken" -- a broken file is silently skipped, so nothing else in this report
            reveals that a layer the user edited never applied.
...
class ConfigDiagnostics(BaseModel):
    ...
    Attributes:
        ...
        runtime_workspace_pin: The workspace directory the active project pinned, when
            that pin is a layer of its own rather than a value read back out of the
            config files. No file holds it, so a settings write cannot reach it.
