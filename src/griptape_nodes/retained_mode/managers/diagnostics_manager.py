...
class DiagnosticsManager(EngineScoped):
    ...
    def _build_config_section(self, redactor: Redactor) -> ConfigDiagnostics:
        """Report the merged settings, the layers behind them, and the env vars overriding them."""
        config_manager = self.engine.config_manager

        layers = config_manager.config_layers()

        return ConfigDiagnostics(
            files=self._config_file_layers(layers, redactor),
            runtime_workspace_pin=self._runtime_workspace_pin(layers, redactor),
        ...
        for layer in layers:
            ...
            if exists:
                try:
                    ...
                    logger.debug("Could not read the size of config file '%s'.", path, exc_info=True)
            files.append(
                ConfigFileDiagnostics(
                    path=redactor.redact_path(path),
                    layer=layer.layer,
                    exists=exists,
                    contributes=layer.present,
