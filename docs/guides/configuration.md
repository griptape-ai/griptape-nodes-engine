...
1. **Configuration Files (`griptape_nodes_config.json`)**
    ...
    - **Load Order (lower numbers are loaded first; higher numbers override):**
        ...
        1. **Per-project workspace override** — When the active project's path matches a key in the `project_workspaces` mapping (in your user config), that workspace directory is applied. This sets only `workspace_directory`, overriding any value from the config files above. No file holds it, so it appears as the `runtime` layer in `gtn self info` and a settings edit to `workspace_directory` cannot change it while it is active. See [Workspace](projects/workspace.md#per-project-workspace-overrides) for details.
