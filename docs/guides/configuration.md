...
1. **Configuration Files (`griptape_nodes_config.json`)**
    ...
    - **Load Order (lower numbers are loaded first; higher numbers override):**
        ...
        1. **Per-project workspace override** — When the active project's path matches a key in the `project_workspaces` mapping (in your user config), that workspace directory is applied. This sets only `workspace_directory`, overriding any value from the config files above. No file holds it, so it appears as the `runtime` layer in `gtn self info` and a settings edit to `workspace_directory` cannot change it while it is active. A project template's own `workspace_dir`, and a workspace inherited from a parent project, are applied the same way. Opening a project that declares none of these pins the workspace directory your user config already specifies, which changes nothing about who owns the setting: it still reports as your user config, and editing it still takes effect the next time you open a project. See [Workspace](projects/workspace.md#per-project-workspace-overrides) for details.
