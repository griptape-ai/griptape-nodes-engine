...
saved, when nodes are copied, when a workflow is embedded in an exported image, and when a node
runs in its library's own process (see [Node Isolation with Workers](node_isolation_with_workers.md)).
...
A value with no plain-data form is left out of saved workflows, copied nodes, and exported images,
so its node runs again to produce it. Sent to or from a library's own process, it fails the node with an error that
