...
```jsonc
{
  ...
  "artifact": {
    "path_at_save": "/Users/x/ws/renders/hero_1.png", // post collision-walk, post extension-coercion
...
No store-level lock is implemented (revised 2026-09-24; the original design penciled a per-record-directory `KeyedMutex` on the preview system's pattern). Review of the shipped pipeline found the store's own writes contention-free without one:

- **Record files never contend.** Names embed a microsecond timestamp plus a random suffix, so two captures — even of the same artifact in the same microsecond — write distinct files. By-hash pointers inherit the same uniqueness.
- **Record and pointer writes go through the engine's private-sibling atomic write**, so a concurrent reader (scanner, inspector) never observes a truncated YAML file.
- **The read-latest-parent window is advisory, not load-bearing.** A racing capture could link a source to a record that is superseded a moment later; the `SourceLink` also carries the source's `content_hash`, which is the durable identity. A momentarily-stale `record_id` link is the same class of drift as `matches_latest_record` going false after capture.

The remaining theoretical window is two simultaneous OVERWRITE saves of the *same artifact path* interleaving their pre-capture/replace sequences (reachable because `save_static_file` runs on worker threads). The artifact itself is already last-rename-wins by design; the worst provenance outcome is two truthful records whose order of arrival differs from the order of the renames. **Awaits evidence:** a demonstrated interleaving that produces a record describing bytes the file never held. If that surfaces, the fix is the originally-penciled `KeyedMutex` keyed on `canonicalize_for_identity` of the record directory, scoped to the pre-capture window.
...
| -------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GetProvenanceForArtifactRequest(macro_path, record_id=None)`                                                                                | "What made this file?"                                                                                                                                                                                         | Latest (or specific) record; `macro_path` accepts macro-form, workspace-relative, or absolute spellings; `is_stale` compares live bytes to the record hash; **hash fallback** on a path miss. No legacy-sidecar fallback: the old sidecars are an evolutionary dead-end, unread by the new system. |
| `ListProvenanceRecordsForArtifactRequest(macro_path)`                                                                                        | "Every iteration of this file"                                                                                                                                                                                 | Directory listing, newest first — the overwrite-same-file history.                                                                                                                                                                                                                                 |
...
| `ListProvenancedArtifactsRequest()`                                                                                                          | The store inventory: every artifact with provenance — portable macro path, artifact kind, record count, latest record ID — built from directory structure alone (no record parsing), sorted by kind then path. |                                                                                                                                                                                                                                                                                                    |
...
- **Existing `.griptape-nodes-metadata` sidecars are never converted** — they lack hashes, IDs, and immutability semantics, and conversion would fabricate identity. The old sidecars are an evolutionary dead-end (ruled 2026-09-23): the query API does not read them at all.
...
| CREATE_NEW collision walk (`hero.png` → `hero_1.png`) | Record keys on the walked final name; the situation block's macro + variables preserve the request intent. Cross-file iteration history is a query (`producing_node_name` filter), not a storage concern.                                                                                                                                                                                   |
| Same bytes re-saved                                   | New record (per-save-event is the contract); `by-hash/` groups them naturally.                                                                                                                                                                                                                                                                                                              |
| Artifact saved outside the workspace                  | Mirrors under `{drive_volume_mount}` — no collisions (retires the old sidecar defect). `macro_path: null`, `situation: null`.                                                                                                                                                                                                                                                               |
...
- **Layout**: collision-walk and extension-coercion saves land records under final names with `extension_coerced_from` populated; outside-workspace saves mirror under `{drive_volume_mount}`; by-hash pointers resolve to canonical records.
